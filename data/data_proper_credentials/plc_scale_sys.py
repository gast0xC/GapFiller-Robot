    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        try:
            ...
            if phase == 1:
                ...
                if not wait_for_axis_free(s): return       
                send_cmd(s, "position47", positions["position47"])  
            if phase == 2:
                time.sleep(0.5)  # Added delay
                send_cmd(s, "cmd_HomeReturn", commands["cmd_HomeReturn"])
                if not wait_for_axis_free(s): return  
                ...
                sem1.release()
                threshold_queue.put(0.3)
                send_cmd(s, "cmd_DispenserStartDirectOn17", commands["cmd_DispenserStartDirectOn"])
                send_cmd(s, "position36", positions["position36"])
                if not wait_for_axis_free(s): return  

                sem2.acquire()
                send_cmd(s, "cmd_DispenserStartDirectOff17", commands["cmd_DispenserStartDirectOff"])
                time.sleep(1)
                send_cmd(s, "position35", positions["position35"])
                #####################################################################################################
                if not wait_for_axis_free(s): return  
                send_cmd(s, "position37", positions["position37"])
                if not wait_for_axis_free(s): return  
                
                sem1.release()
                threshold_queue.put(0.3)
                send_cmd(s, "cmd_DispenserStartDirectOn18", commands["cmd_DispenserStartDirectOn"])
                send_cmd(s, "position38", positions["position38"])
                if not wait_for_axis_free(s): return  

                sem2.acquire()
                send_cmd(s, "cmd_DispenserStartDirectOff18", commands["cmd_DispenserStartDirectOff"])
                time.sleep(1)
                send_cmd(s, "position37", positions["position37"])
                #####################################################################################################
                if not wait_for_axis_free(s): return  
                send_cmd(s, "position39", positions["position39"])
                if not wait_for_axis_free(s): return  
                
                sem1.release()
                threshold_queue.put(0.3)
                send_cmd(s, "cmd_DispenserStartDirectOn19", commands["cmd_DispenserStartDirectOn"])
                send_cmd(s, "position40", positions["position40"])
                if not wait_for_axis_free(s): return  

                sem2.acquire()
                send_cmd(s, "cmd_DispenserStartDirectOff19", commands["cmd_DispenserStartDirectOff"])
                time.sleep(1)
                send_cmd(s, "position39", positions["position39"])
                #####################################################################################################
                if not wait_for_axis_free(s): return  
                send_cmd(s, "position41", positions["position41"])
                if not wait_for_axis_free(s): return  
                
                sem1.release()
                threshold_queue.put(0.3)
                send_cmd(s, "cmd_DispenserStartDirectOn20", commands["cmd_DispenserStartDirectOn"])
                send_cmd(s, "position42", positions["position42"])
                if not wait_for_axis_free(s): return  

                sem2.acquire()
                send_cmd(s, "cmd_DispenserStartDirectOff20", commands["cmd_DispenserStartDirectOff"])
                time.sleep(1)
                send_cmd(s, "position41", positions["position41"])
                #####################################################################################################
                if not wait_for_axis_free(s): return  
                send_cmd(s, "position43", positions["position43"])
                if not wait_for_axis_free(s): return  
                
                sem1.release()
                threshold_queue.put(0.3)
                send_cmd(s, "cmd_DispenserStartDirectOn21", commands["cmd_DispenserStartDirectOn"])
                send_cmd(s, "position44", positions["position44"])
                if not wait_for_axis_free(s): return  

                sem2.acquire()
                send_cmd(s, "cmd_DispenserStartDirectOff21", commands["cmd_DispenserStartDirectOff"])
                time.sleep(1)
                send_cmd(s, "position43", positions["position43"])
                #####################################################################################################
                if not wait_for_axis_free(s): return  
                send_cmd(s, "position45", positions["position45"])
                if not wait_for_axis_free(s): return  
                
                sem1.release()
                threshold_queue.put(0.3)
                send_cmd(s, "cmd_DispenserStartDirectOn22", commands["cmd_DispenserStartDirectOn"])
                send_cmd(s, "position46", positions["position46"])
                if not wait_for_axis_free(s): return  

                sem2.acquire()
                send_cmd(s, "cmd_DispenserStartDirectOff22", commands["cmd_DispenserStartDirectOff"])
                time.sleep(1)
                send_cmd(s, "position45", positions["position45"])
    ...
    try:
       ...
       # if phase == 1:
        ...
        if phase == 2:
            ...
            with open(file_path, 'w') as file:
                pass
            res = finalWeight - initialWeightaux - 7.4 # o IMS e' entre [8.6 ; 8.8]
