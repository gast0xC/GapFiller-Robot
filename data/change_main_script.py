...
def generate_commands(num_points: int, thresholds: List[float], phase: int) -> List[str]:
    ...
    if phase == 1:
        start_point = 3  # Skipping the first 2 points
        end_point = phase_1_end
    elif phase == 2:
        start_point = phase_2_start
        end_point = num_points - 1  # Skipping the last point
    else:
...
def generate_thread_func_script(file_path, num_points, thresholds):
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        try:
            ...
            if phase == 1:
                ...
                if not wait_for_axis_free(s): return       
                send_cmd(s, "position{num_points}", positions["position{num_points}"])  
            if phase == 2:
                time.sleep(0.5)  # Added delay
                send_cmd(s, "cmd_HomeReturn", commands["cmd_HomeReturn"])
                if not wait_for_axis_free(s): return  
    ...
    # Replace the existing thread_one_func with the new one
    new_file_contents = re.sub(
        r'def thread_one_func\(.*?\):.*?terminate_flag\.set\(\)',
        thread_func_content,
        file_contents,
        flags=re.DOTALL
    )
